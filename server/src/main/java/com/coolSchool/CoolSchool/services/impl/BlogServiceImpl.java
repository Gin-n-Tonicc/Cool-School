...
public class BlogServiceImpl implements BlogService {
    ...
    public BlogResponseDTO getBlogById(Long id, PublicUserDTO loggedUser) {
        if (loggedUser != null) {
            if (loggedUser.getRole().equals(Role.ADMIN)) {
                ...
                return modelMapper.map(blog.get(), BlogResponseDTO.class);
            }
        }
    ...
    public BlogResponseDTO createBlog(BlogRequestDTO blogDTO, PublicUserDTO loggedUser) {
        ...
        try {
            ...
            blogDTO.setOwner(loggedUser.getId());
            if (loggedUser.getRole().equals(Role.ADMIN)) {
                blogDTO.setEnabled(blogDTO.isEnabled());
            } else {
                blogDTO.setEnabled(false);
            }
            userRepository.findByIdAndDeletedFalse(blogDTO.getOwner()).orElseThrow(UserNotFoundException::new);
            categoryRepository.findByIdAndDeletedFalse(blogDTO.getCategory()).orElseThrow(CategoryNotFoundException::new);
            if (blogDTO.getPicture() != null) {
                fileRepository.findByIdAndDeletedFalse(blogDTO.getPicture()).orElseThrow(FileNotFoundException::new);
            }
    ...
    public BlogResponseDTO updateBlog(Long id, BlogRequestDTO blogDTO, PublicUserDTO loggedUser) {
        ...
        blogRepository.findByIdAndDeletedFalseIsEnabledTrue(blogDTO.getCategory()).orElseThrow(BlogNotFoundException::new);
        fileRepository.findByIdAndDeletedFalse(blogDTO.getPicture()).orElseThrow(FileNotFoundException::new);
